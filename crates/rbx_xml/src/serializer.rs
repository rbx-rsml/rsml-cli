...
pub fn encode_internal<W: Write>(
    ...
    for id in ids {
        let id = *id;
        serialize_instance(&mut writer, &mut state, tree, id, true, &mut property_buffer)?;
    }
...
fn serialize_instance<'dom, W: Write>(
    ...
    let mapped_id_str = if force_id {
        id.to_string()
    } else {
        state.map_id(id).to_string()
    };

    writer.write(
        XmlWriteEvent::start_element("Item")
            .attr("class", &instance.class)
            .attr("referent", &mapped_id_str),
    )?;
    ...
    for child_id in instance.children() {
        serialize_instance(writer, state, tree, *child_id, true, property_buffer)?;
    }
